...
function ShowEditBotsPage()
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		// POST Handler für Redistribute (manuell hinzugefügt - in Zeile 111 einfügen!)

		if (isset($_POST['redistribute_personalities'])) {
    		$sql = "UPDATE " . DB_PREFIX . "bots 
            SET personality = ELT(FLOOR(1 + RAND() * 6), 'balanced', 'farmer', 'raider', 'researcher', 'miner', 'turtle')";
   		 $db->query($sql);
    		$message = ['class' => 'success', 'text' => 'Persönlichkeiten wurden neu verteilt!'];
    		$mode = 'personalities';
		}
    ...
    switch ($mode) {
        ...
        case 'personalities':
            $tplData['personalities'] = getPersonalitiesData($db);
            break;
...
function getPersonalitiesData($db): array
{
    // Hole alle Personalities aus der Tabelle
    $personalities = $db->select("SELECT * FROM " . DB_PREFIX . "bot_personalities ORDER BY name");
    
    // Zähle Bots pro Personality
    foreach ($personalities as &$p) {
        $count = $db->selectSingle(
            "SELECT COUNT(*) as c FROM " . DB_PREFIX . "bots WHERE personality = :name",
            [':name' => $p['name']],
            'c'
        );
        $p['bot_count'] = (int)$count;
    }
